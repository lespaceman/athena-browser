 ...
 * MCP Testing Helpers
 *
 * Utility functions and types for testing MCP servers.
 */
...
/**
 * Create an MCP client connected to the athena-agent server
 */
...
export function validateToolSchema(
  ...
  for (const prop of expectedProperties) {
    if (!(prop in tool.inputSchema.properties)) {
      throw new Error(`Tool '${expectedName}' missing property '${prop}'`);
    }
  }
}
