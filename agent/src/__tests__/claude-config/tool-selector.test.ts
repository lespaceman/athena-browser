...
describe('ToolSelector', () => {
  describe('selectTools', () => {
    it('should return empty array when MCP disabled', () => {
      const tools = ToolSelector.selectTools('show me the page', false);
      expect(tools).toEqual([]);
    });

    it('should return read-only tools for "show" prompt', () => {
      const tools = ToolSelector.selectTools('show me the page', true);
    ...
    it('should return read-only tools for "get" prompt', () => {
      const tools = ToolSelector.selectTools('get the page title', true);
      expect(tools.length).toBeGreaterThan(0);
      expect(tools).toContain('mcp__athena-browser__browser_get_page_summary');
    ...
    it('should return read-only tools for "find" prompt', () => {
      const tools = ToolSelector.selectTools('find all links on the page', true);
    ...
    it('should return read-only tools for "search" prompt', () => {
      const tools = ToolSelector.selectTools('search for form elements', true);
      expect(tools).toContain('mcp__athena-browser__browser_query_content');
      expect(tools).not.toContain('mcp__athena-browser__browser_execute_js');
    });

    it('should return read-only tools for "analyze" prompt', () => {
      const tools = ToolSelector.selectTools('analyze the page structure', true);
    ...
    it('should return read-only tools for "extract" prompt', () => {
      const tools = ToolSelector.selectTools('extract data from the page', true);
      expect(tools).toContain('mcp__athena-browser__browser_query_content');
      expect(tools).not.toContain('mcp__athena-browser__browser_execute_js');
    });

    it('should return interactive tools for "click" prompt', () => {
      const tools = ToolSelector.selectTools('click the submit button', true);
    ...
    it('should return interactive tools for "fill" prompt', () => {
      const tools = ToolSelector.selectTools('fill out the form', true);
      expect(tools).toContain('mcp__athena-browser__browser_execute_js');
    });

    it('should return interactive tools for "submit" prompt', () => {
      const tools = ToolSelector.selectTools('submit the form', true);
      expect(tools).toContain('mcp__athena-browser__browser_execute_js');
    });

    it('should return interactive tools for "execute" prompt', () => {
      const tools = ToolSelector.selectTools('execute some JavaScript', true);
      expect(tools).toContain('mcp__athena-browser__browser_execute_js');
    });

    it('should return interactive tools for "navigate" prompt', () => {
      const tools = ToolSelector.selectTools('navigate to google.com', true);
      expect(tools).toContain('mcp__athena-browser__browser_navigate');
      expect(tools).toContain('mcp__athena-browser__browser_execute_js');
    });

    it('should return interactive tools for "type" prompt', () => {
      const tools = ToolSelector.selectTools('type in the search box', true);
      expect(tools).toContain('mcp__athena-browser__browser_execute_js');
    });

    it('should prioritize interaction over read-only when both keywords present', () => {
      const tools = ToolSelector.selectTools('show me the page and click the button', true);
      expect(tools).toContain('mcp__athena-browser__browser_execute_js');
      expect(tools).toContain('mcp__athena-browser__browser_navigate');
    });

    it('should be case insensitive', () => {
      const tools1 = ToolSelector.selectTools('SHOW me the page', true);
      const tools2 = ToolSelector.selectTools('show me the page', true);
      expect(tools1).toEqual(tools2);
    });

    it('should include tab info tool in read-only mode', () => {
      const tools = ToolSelector.selectTools('get tab information', true);
    ...
    it('should include all tab management tools in interactive mode', () => {
      const tools = ToolSelector.selectTools('navigate to a new tab', true);
