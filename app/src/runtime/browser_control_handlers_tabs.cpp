...
std::string BrowserControlServer::HandleCreateTab(const std::string& url) {
  ...
  if (tab_index < 0) {
    return nlohmann::json{{"success", false}, {"error", "Failed to create tab"}}.dump();
  }

  bool loaded =
      window->WaitForLoadToComplete(static_cast<size_t>(tab_index), kDefaultNavigationTimeoutMs);
...
std::string BrowserControlServer::HandleCloseTab(size_t tab_index) {
  ...
  if (tab_index >= count) {
    return nlohmann::json{{"success", false}, {"error", "Invalid tab index"}}.dump();
  }

  window->CloseTab(tab_index);
  return nlohmann::json{{"success", true}, {"tabIndex", static_cast<int>(tab_index)}}.dump();
}
...
std::string BrowserControlServer::HandleTabInfo() {
  ...
  return nlohmann::json{{"success", true}, {"count", count}, {"activeTabIndex", active}}.dump();
}
